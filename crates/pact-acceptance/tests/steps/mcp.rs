...
async fn given_policy_no_ai_exec(world: &mut PactWorld, _vcluster: String) {
    world.auth_result = Some(AuthResult::Denied {
        reason: "AI exec not authorized for this vCluster".into(),
    });
}
...
async fn when_ai_status(world: &mut PactWorld) {
    ...
    world.cli_exit_code = Some(if result.is_error { 1 } else { 0 });
...
async fn when_ai_query_fleet(world: &mut PactWorld) {
    let result = dispatch_tool(
        "pact_query_fleet",
        &json!({"capability_filter": "gpu_health=degraded"}),
    );
    world.cli_output = Some(result.content[0].text.clone());
    world.cli_exit_code = Some(if result.is_error { 1 } else { 0 });
}
...
async fn when_ai_emergency(world: &mut PactWorld) {
    ...
    if result.is_error {
        ...
        world.auth_result = Some(AuthResult::Denied {
            reason: result.content[0].text.clone(),
        });
    }
}
...
async fn when_ai_exec(world: &mut PactWorld, command: String, node: String) {
    ...
    record_audit(
        ...
        &format!("pact_exec node={} command={}", node, command),
    );
}
...
async fn then_fleet_response(world: &mut PactWorld) {
    ...
    assert!(
        output.contains("gpu_health=degraded"),
        "should query for degraded GPUs"
    );
}
...
async fn then_author_recorded(world: &mut PactWorld, expected: String) {
    let has_ai_author = world
        .journal
        .audit_log
        .iter()
        .any(|op| op.actor.principal == expected);
    assert!(
        has_ai_author,
        "audit log should contain author '{expected}'"
    );
}
...
async fn then_actor_type(world: &mut PactWorld, ptype: String) {
    ...
    let has_type = world
        ...
        .audit_log
        .iter()
        .any(|op| op.actor.principal_type == expected);
