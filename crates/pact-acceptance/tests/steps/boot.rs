...
fn simulate_boot_stream(world: &mut PactWorld, node_id: &str, vcluster: &str) {
    ...
    if let Some(overlay) = world.journal.overlays.get(vcluster) {
        ...
        world
            .journal
            .apply_command(JournalCommand::SetOverlay {
                vcluster_id: vcluster.to_string(),
                overlay,
            });
    }
    ...
    let node_deltas: Vec<_> = world
        ...
        .values()
        .filter(|e| {
            e.entry_type == EntryType::Commit && e.scope == Scope::Node(node_id.into())
        })
        .collect();
    ...
    if !node_deltas.is_empty() {
        ...
        world
            .boot_stream_chunks
            .push(BootStreamChunk::NodeDelta { data: delta_data });
    }

    // ConfigComplete
    let base_version = world
        .journal
        .overlays
        .get(vcluster)
        .map(|o| o.version)
    ...
    world.boot_stream_chunks.push(BootStreamChunk::Complete {
        base_version,
        node_version: if node_deltas.is_empty() {
            None
        } else {
            Some(1)
        },
    ...
    if let Some(ref table) = step.table {
        for row in table.rows.iter().skip(1) {
            ...
            world
                ...
                .push(pact_common::types::ServiceDecl {
                    name,
...
async fn when_config_commit(world: &mut PactWorld, vcluster: String) {
    ...
    world.received_updates.push(ConfigUpdateEvent {
        sequence: seq,
...
async fn when_policy_updated(world: &mut PactWorld, vcluster: String) {
    ...
    world.received_updates.push(ConfigUpdateEvent {
        sequence: seq,
        update_type: "policy_update".into(),
    });
}
...
async fn when_blacklist_updated(world: &mut PactWorld, vcluster: String) {
    ...
    world.received_updates.push(ConfigUpdateEvent {
        sequence: seq,
        update_type: "blacklist_update".into(),
    });
}
...
async fn then_start_first(world: &mut PactWorld, name: String) {
    ...
    assert_eq!(sorted.first().map(|s| s.as_str()), Some(name.as_str()));
}
...
async fn then_start_second(world: &mut PactWorld, name: String) {
    ...
    assert_eq!(sorted.get(1).map(|s| s.as_str()), Some(name.as_str()));
}
...
async fn then_start_last(world: &mut PactWorld, name: String) {
    ...
    assert_eq!(sorted.last().map(|s| s.as_str()), Some(name.as_str()));
}
...
async fn then_subscribed(world: &mut PactWorld) {
    assert!(
        !world.subscriptions.is_empty(),
        "agent should have a config subscription"
    );
}
...
async fn then_boot_normal(world: &mut PactWorld) {
    assert!(
        !world.boot_phases_completed.is_empty(),
        "boot should have completed phases"
    );
}
...
async fn then_cached_config(world: &mut PactWorld) {
    ...
    assert!(
        ...
        "should have boot stream chunks from cache"
    );
}
...
async fn then_retry_journal(world: &mut PactWorld) {
    assert!(
        ...
        "journal should still be unreachable (retry is background)"
    );
}
...
async fn then_overlay_data_matches(world: &mut PactWorld) {
    ...
    if let Some(BootStreamChunk::BaseOverlay { data, .. }) = world
        .boot_stream_chunks
        .iter()
        .find(|c| matches!(c, BootStreamChunk::BaseOverlay { .. }))
    {
...
async fn then_overlay_rebuilt(world: &mut PactWorld, vcluster: String) {
    assert!(
        ...
        "overlay should exist for {vcluster}"
    );
}
...
async fn then_overlay_version_gt(world: &mut PactWorld, min_version: u64) {
    ...
    assert!(
        ...
        "overlay version {max_version} should be > {min_version}"
    );
}
...
async fn then_update_notification(world: &mut PactWorld, node: String) {
    assert!(
        world.subscriptions.contains_key(&node),
        "node should be subscribed"
    );
    assert!(
...
async fn then_policy_notification(world: &mut PactWorld, node: String) {
    ...
    assert!(world
        .received_updates
        .iter()
        .any(|u| u.update_type == "policy_update"));
}
...
async fn then_blacklist_notification(world: &mut PactWorld, node: String) {
    ...
    assert!(world
        .received_updates
        .iter()
        .any(|u| u.update_type == "blacklist_update"));
}
