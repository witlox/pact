...
use pact_policy::federation::{FederationError, FederationState, MockFederationSync, FederationSync, SyncResult};
...
async fn when_template_synced(world: &mut PactWorld) {
    ...
    state.templates = world.federated_templates.clone();
...
async fn when_compliance_report(world: &mut PactWorld) {
    ...
    let drift_count = world
        .journal
        .entries
        .values()
        .filter(|e| e.entry_type == EntryType::DriftDetected)
        .count();
    let audit_count = world.journal.audit_log.len();

    world.compliance_reports.push(format!(
        "Compliance report: {} drift events, {} audit entries",
        drift_count, audit_count
    ));
}
...
async fn then_local_only(world: &mut PactWorld) {
    assert!(
        ...
        "entry should be in local journal"
    );
}
...
async fn then_report_sent(world: &mut PactWorld) {
    assert!(
        !world.compliance_reports.is_empty(),
...
async fn then_new_templates_loaded(world: &mut PactWorld) {
    assert!(
        !world.federated_templates.is_empty(),
        "new templates should be loaded"
    );
}
