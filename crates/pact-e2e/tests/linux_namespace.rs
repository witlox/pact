...
#![cfg(target_os = "linux")]
#![allow(unused_variables)] // Command outputs used conditionally in early-return tests
...
async fn mount_namespace_isolation() {
    ...
    let output = Command::new("unshare")
        .args([
            ...
            "sh",
            "-c",
            "mount -t tmpfs none /tmp/ns-test-mount && echo MOUNTED",
        ])
        .output()
    ...
    assert!(!mounts.contains("ns-test-mount"), "mount should be isolated to child namespace");
}
...
async fn pid_namespace_isolation() {
    ...
    assert!(stdout.contains("Pid:"), "should see Pid in process status, got: {stdout}");
}
...
async fn network_namespace_isolation() {
    ...
    assert!(stdout.contains("lo"), "new net namespace should have loopback, got: {stdout}");
...
async fn squashfs_mount_lifecycle() {
    ...
    let output = Command::new("mksquashfs")
        .args([source_dir.to_str().unwrap(), image_path.to_str().unwrap(), "-noappend", "-quiet"])
        .output()
        .expect("mksquashfs");
    ...
    // Unmount
    let output =
        Command::new("umount").arg(mount_point.to_str().unwrap()).output().expect("umount");
